...
def index2num(index):
    ...
    sum_num = -1
    for i in range(65):
        dir_path = '/home/hhy/simulation_data/patches/' + str(i) + '/'
...
def get_testdata(index, model,save_path):
    ...
    if num == model:
        ...
        if model == 65:
            model -= 2
    pose_file = open('/home/hhy/simulation_data3.0/grasps_pose.txt', 'rb')
    poses = pickle.load(pose_file)
    pose_file.close()
    pose = get_pose(poses[index]) #手掌相对graspit世界坐标系的矩阵T
    zxyzw = []
    ...
    sdf_path = '/home/hhy/simulation_data3.0/gazebo_data/sdf_file/o' + str(model) + '.sdf'
    wTo = get_wTo(sdf_path)
    center, angle = pose2patch(pose, wTo)

    #origin_depth = '/home/hhy/simulation_data3.0/depth/' + str(num) + '.jpg'
    depth_path = '/home/hhy/simulation_data3.0/depth/' + str(model) + '.jpg'
    patch_size = [128, 64]
    #o_img = get_patch(origin_depth, angle, center, patch_size)
    img = get_patch(depth_path, angle, center, patch_size)
    img.save(save_path)
    #o_img.save('/home/hhy/simulation_data3.0/test/origin.jpg')
...
if __name__=='__main__':
    ...
    o_img = get_patch(origin_depth, angle, center, patch_size)
    img = get_patch(depth_path, angle, center, patch_size)
    save_path = '/home/hhy/simulation_data3.0/test/0.jpg'
    img.save(save_path)
    o_img.save('/home/hhy/simulation_data3.0/test/origin.jpg')
