...
def main():
    ...
    target_states = [1.4004536867141724, -0.832428280507223, 
                     1.8243780136108398, 1.4410158395767212, 
                     -0.6823609511004847, 0.2685181796550751]

    # target-1
    # target_states[0] += 0.06
    # target_states[1] += 0.02
    # target_states[4] = origin_states[4] + 0.80

    # target-2
    target_states[0] += 0.08
    target_states[1] += 0.03
    target_states[4] = origin_states[4] + 0.80

    # target_states = list(origin_states)
    # target_states[0] -= 0.15
    # target_states[1] += 2.07
    # target_states[2] -= 0.75
    # target_states[3] -= 0.55
    # target_states[4] += 0.90
    # target_states[5] += 0.23


    high_states = list(target_states)
    high_states[1] -= 0.01

    # ur_script_pub = rospy.Publisher('/ur5/ur_driver/URScript', String, queue_size=10)
    while True:
        print 'Input o to move arm to origin, t to target, h to higher, s to stop'
        key_word = raw_input()
        if key_word == 'o':
            ...
            group.set_joint_value_target(target_states)
            plan = group.plan()
            group.execute(plan)
        elif key_word == 'h':
            # group.move_to_joint_value_target(high_states)
            group.set_joint_value_target(high_states)
            plan = group.plan()
            group.execute(plan)
        # elif key_word == 'ty':
